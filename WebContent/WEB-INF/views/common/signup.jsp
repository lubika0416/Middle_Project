...
<script>
	$(function() {
		...
		arg1.on('change', function() {
			if (arg1.is(":checked") && arg2.is(':checked')) {
				$('.join1').removeAttr('disabled');
			} else {
				$('.join1').attr('disabled', true);
			}
		})
		
		arg2.on('change', function() {
			if (arg1.is(":checked") && arg2.is(':checked')) {
				$('.join1').removeAttr('disabled');
			} else {
				$('.join1').attr('disabled', true);
...
<body>
	...
	<div class="container px-lg-5 pb-5">
		...
		<div class="row mt-5">
			<textarea class="form-control" rows="15" style="resize: none;">제1조(목적) 이 약관은 업체 회사(전자상거래 사업자)가 운영하는 업체 사이버 몰(이하 “몰”이라 한다)에서 제공하는 인터넷 관련 서비스(이하 “서비스”라 한다)를 이용함에 있어 사이버 몰과 이용자의 권리․의무 및 책임사항을 규정함을 목적으로 합니다.
...
제24조(재판권 및 준거법)
  ...
  ② “몰”과 이용자 간에 제기된 전자상거래 소송에는 한국법을 적용합니다.
		...
		<div class="row">
			<div class="form-check" style="margin-top: 15px; margin-bottom: 30px">
				
				<label class="form-check-label" for="flexCheckChecked">
					<input class="form-check-input arg1" type="checkbox" value="1" id="flexCheckChecked"> 이용약관에 동의합니다.
				</label> 
			</div>
		</div>
		<div class="row">
		...
		<div class="row">
			<div class="form-check" style="margin-top: 15px; margin-bottom: 30px">
				...
				<label class="form-check-label" for="flexCheckChecked1">
					<input class="form-check-input arg2"  type="checkbox" value="1" id="flexCheckChecked1"> 이용약관에 동의합니다.
				</label> 
			</div>
		</div>
		<div style="display: flex; justify-content: space-evenly;">
			<button type="submit" class="btn btn-primary btn-lg join1" disabled onclick="location.href='member/signup.do'">일반 회원 가입</button>
			<button type="submit" class="btn btn-success btn-lg join1" disabled onclick="location.href='producer/signup.do'">사업자 회원 가입</button>
		</div>
	</div>
