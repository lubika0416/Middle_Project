...
<div class="mypagimargin" style="padding: 8px;">
	<div class="calbox">
		<div class="mycontenttitle">관리자 문의 내역</div>
		...
		<div class="table-responsive small">
			<table class="table table-lm">
				...
				<tbody>
					...
					%>
					<tr>
						<td><%=myaqList.get(i).getAqnaNo()%></td>
						<td><a
							href="AQNAAnswerView.do?aqnaNo=<%=myaqList.get(i).getAqnaNo()%>"><%=myaqList.get(i).getAqnaTitle()%>
						</a></td>
						<td><%=myaqList.get(i).getFormAqnaRegdt()%></td>
						<%
							if (Integer.parseInt(myaqList.get(i).getAqnaStatus()) == 0) {
						%>
						<td>답변안됨</td>
						<%
							} else {
